...
let setLabelSize = function (chart, data, options) {
  let maxLabelSize = Math.ceil(
    chart.element.offsetWidth / 4.0 / data.labels.length
  );
  if (maxLabelSize > 25) {
    maxLabelSize = 25;
...
let jsOptions = jsOptionsFunc(
  merge(baseOptions, defaultOptions),
  hideLegend,
  setTitle,
  setMin,
  setMax,
...
let createDataTable = function (chart, options, chartType, library) {
  ...
  if (
    chartType === "bar" ||
    chartType === "column" ||
    (chart.xtype !== "number" && chart.xtype !== "bubble")
  ) {
    let sortedLabels = [];
  ...
  if (chart.xtype === "datetime" && labels.length > 0) {
    ...
    if (!options.scales.xAxes[0].time.unit) {
      ...
      if (step && timeDiff > 0) {
        let unitStepSize = Math.ceil(
          timeDiff / step / (chart.element.offsetWidth / 100.0)
        );
        if (week && step === 1) {
...
export default class {
  ...
  renderLineChart(chart, chartType) {
    ...
    if (chart.xtype === "number") {
      ...
      options.scales.xAxes[0].type =
        chart.xtype === "string" ? "category" : "time";
    }
  ...
  drawChart(chart, type, data, options) {
    ...
    if (chart.options.code) {
      window.console.log(
        "new Chart(ctx, " + JSON.stringify(chartOptions) + ");"
      );
    }
